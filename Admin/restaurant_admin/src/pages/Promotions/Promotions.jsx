...
const Promotions = () => {
  ...
  const fetchPromotions = async () => {
    try {
      ...
      setError('Không thể tải danh sách khuyến mãi: ' + (err.response?.data || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchCategories = async () => {
    try {
      ...
      setError('Không thể tải danh sách loại món: ' + (err.response?.data || err.message));
    }
  };
  ...
  const fetchStats = async (id) => {
    try {
      ...
      setError('Không thể tải thống kê: ' + (err.response?.data || err.message));
    }
  };
      ...
      if (status === 400) {
        if (data && typeof data === 'object') {
          ...
          if (data.title === 'One or more validation errors occurred.') {
            ...
            return validationErrors.length > 0 ? validationErrors : ['Dữ liệu không hợp lệ'];
          }
          // Xử lý các lỗi khác từ server
          return [data.message || 'Dữ liệu không hợp lệ'];
        }
        return ['Dữ liệu không hợp lệ: ' + (typeof data === 'string' ? data : 'Vui lòng kiểm tra lại thông tin')];
      }
  ...
  const validateField = (name, value) => {
    switch (name) {
      case 'tenKhuyenMai':
        ...
        if (value.trim().length < 3) return 'Tên khuyến mãi phải có ít nhất 3 ký tự';
        if (value.trim().length > 100) return 'Tên khuyến mãi không được vượt quá 100 ký tự';
        return '';
      ...
      case 'dieuKien':
        ...
        if (value.trim().length > 500) return 'Điều kiện không được vượt quá 500 ký tự';
        return '';
  ...
  const handleAddPromotion = async (e) => {
    ...
    if (!validateForm()) {
      ...
      const firstErrorField = document.querySelector('[style*="border: 1px solid #d32f2f"]');
      if (firstErrorField) {
    ...
    try {
      ...
      if (err.response?.data) {
        if (typeof err.response.data === 'object') {
          ...
          if (err.response.data.title === 'One or more validation errors occurred.') {
            ...
            // Xử lý các lỗi khác từ server
            errorMessages.push(err.response.data.message || 'Dữ liệu không hợp lệ');
          }
      ...
      if (errorMessages.length === 0) {
        errorMessages = ['Không thể thêm khuyến mãi. Vui lòng kiểm tra lại thông tin.'];
    }
  ...
  const handleEditPromotion = async (e) => {
    ...
    try {
      ...
      setError('Không thể cập nhật khuyến mãi: ' + (err.response?.data || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeletePromotion = async (id) => {
    ...
    try {
      ...
      setError('Không thể xóa khuyến mãi: ' + (err.response?.data || err.message));
    } finally {
      setLoading(false);
    }
  };
            ...
            <div key={promotion.maKhuyenMai} className="promotion-card">
              <div className="promotion-header">
                <h3>
                  ...
                  <span className={`status-badge ${promotion.trangThai ? 'status-active' : 'status-inactive'}`}>
              ...
              <div className="promotion-details">
                ...
                <p>
                  ...
                  <span className="discount-badge">{promotion.phanTramGiam}%</span>
                </p>
                <p>
                  <strong>Thời gian:</strong>
                  <span>
                    {new Date(promotion.ngayBatDau).toLocaleDateString('vi-VN')} - {new Date(promotion.ngayKetThuc).toLocaleDateString('vi-VN')}
                  </span>
                </p>
                <p>
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleAddPromotion}>
              ...
              <div className="form-group">
                ...
                <div className={`category-selection ${fieldErrors.maLoaiMon ? 'error' : ''}`}>
                  {categories.length === 0 ? (
                    ...
                    <div className="category-grid">
                      {categories.map(category => (
                        <label 
                          key={category.maLoai}
                          className={`category-item ${selectedCategories.includes(Number(category.maLoai)) ? 'selected' : ''}`}
                        >
                          <input
                            type="checkbox"
                            checked={selectedCategories.includes(Number(category.maLoai))}
                            onChange={(e) => handleCategoryChange(category.maLoai, e.target.checked)}
                          />
                  ...
                  <div className="selected-categories">
                    ...
                    <div className="category-tags">
                      {selectedCategories.map(ma => {
                        const category = categories.find(c => Number(c.maLoai) === ma);
                        return category ? (
                          <span key={ma} className="category-tag">
                            {category.tenLoai}
                          </span>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const newSelected = e.target.checked
                                ? [...selectedCategories, Number(category.maLoai)]
                                : selectedCategories.filter(id => id !== Number(category.maLoai));
                              setSelectedCategories(newSelected);
                              const error = validateField('maLoaiMon', newSelected);
                              setFieldErrors(prev => ({
                                ...prev,
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <div className="stats-header">
              ...
              <span className={`status-badge ${currentPromotion.trangThai ? 'status-active' : 'status-inactive'}`}>
