        ...
        <div className="payment-popup-container">
          ...
          <div className="payment-popup-options">
            {paymentOptions.map((option, idx) => (
              <button
                key={option.label}
                className={`payment-popup-option${selected === idx ? ' selected' : ''}`}
          ...
          <div className="payment-popup-qr-section">
            <img src={paymentOptions[selected].img} alt={paymentOptions[selected].alt} className="payment-popup-qr-img" />
            <p>{paymentOptions[selected].description}</p>
          </div>
          <button className="payment-popup-confirm-button" onClick={() => setShowConfirm(true)}>I have done my payment</button>
        </div>
      </div>
      <PaymentConfirm show={showConfirm} onClose={() => { setShowConfirm(false); onClose(); }} />
    </>
  );
};
